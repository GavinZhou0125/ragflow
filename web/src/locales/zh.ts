...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    setting: {
      ...
      mcpserver: 'MCP 服务器',
      mcpServerSettingsDescription: '在此管理可用的 MCP 服务器。',
      ...
      mcpServerUrl: 'URL',
      mcpServerVariables: '变量',
      mcpServerVariablesTip: '定义可以在 HTTP 请求头中使用的变量',
      mcpServerVariableKey: '变量 Key',
      mcpServerVariableName: '变量名称',
      mcpServerHeaders: 'HTTP 请求头',
      mcpServerHeadersTip:
        '定义将会被发送到 MCP 服务器的请求头。可以用 ${key} 的形式引用变量',
      mcpServerSureDelete: '您确定要删除该 MCP 服务器吗？',
      team: '团队',
      system: '系统',
      logout: '登出',
      api: 'API',
...
以上就是你需要总结的内容。`,
      ...
      useLatestMessageOnlyTip:
        '如果你使用了对话组件的输出，那么启用该选项将仅使用对话组件中的最新一条消息进行检索。',
      code: '代码',
      ...
      modelMcpServers: 'MCP 服务器',
      mcpInputVariable: 'MCP 服务器输入变量映射',
      query: '查询变量',
      agent: 'Agent',
